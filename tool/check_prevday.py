...
nowsec=datetime.datetime.now().timestamp()
for sym,name in SYMS:
    ...
    try:
        ...
        last=series[-1]
        stateOpen=(state=="REGULAR")
        sessionOpen=("start" in reg and "end" in reg and reg["start"]<=nowsec<reg["end"])
        inprog=(last[0]==today and (stateOpen or sessionOpen))
        idx=len(series)-2 if inprog else len(series)-1
        if idx<1: idx=len(series)-1
        cur=series[idx]; prev=series[idx-1]
        chg=cur[2]-prev[2]; pct=chg/prev[2]*100
        print(f"{name:7s} state={state or '-':8s} sessOpen={sessionOpen} 선택={ds(cur[1]+gmt)} close={round(cur[2],2)} ({round(pct,2)}%)")
